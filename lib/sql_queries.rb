...
"SELECT projects.title, SUM(pledges.amount) from projects JOIN pledges on projects.id = pledges.project_id GROUP BY projects.title ORDER BY title;"
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"SELECT users.name, users.age, SUM(pledges.amount) FROM users JOIN pledges ON user_id = users.id GROUP BY users.name ORDER BY name;"
end
